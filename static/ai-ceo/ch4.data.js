/* ══════════════════════════════════════════════════════════════════════════
   ch4.data.js ── 第4章「正誤で鍛える」の数値と式（★設計役の持ち物）
   ──────────────────────────────────────────────────────────────────────────
   ...
   役目   カード定義・値札・必要人員・バーの加算ポイント・第4章の決算式・出典
   ★置かないもの  解説の本文・画面の文言（ch4.html＝実装役の持ち物）／
                  ★章をまたぐ条件（→04 §11 の一覧。★土台が持つ）
   ★fetch を使わない（→01 §9・§10・04 §0）
   ──────────────────────────────────────────────────────────────────────────
   ★第4章の性質（→02 §4-F-3・§4-F-4・§4-F-5）
     ・★代償は金ではなく【人】である。通期40名の枠を3本（見本・好み訓練・検証器）で配る
       → 満額で取るには 10＋30＋30 ＝ 70名 必要。★上限40名なので全部は取れない
     ・★「報酬モデルを使う」はカードにしない（第24版で外した。★二重の経路だった）
     ・★RLVR を取ると第5章が変わる（例を見せる の符号が反転／道具が盤面から消える）
       → ★これは【章をまたぐ】ので、この data には入れない（→04 §11 の①）
   ══════════════════════════════════════════════════════════════════════════ */
...
/* ── カード（第4章。02 §4-F-2・§4-F-3・§4-F-4）──────────────────────
   staff : 必要な人員（通期40名の枠から引く）  think/obey : バーへの加算（設定29・30）
   turn  : ★第2〜5章はターンで縛らない（02 設定37。→04 §11-3 の①）
   ★★tag と cite は【画面に出る文字列】である。★★・02／04 への参照・版番号を書かない
     （→03 §36-0 の規約）。★設計の注記はコメント側に置く                          */
cards:[
 ...
 /* ★人の模範解答を先に入れる ── ★大きさまで実測がある唯一の罠（AIME 77.9 → 59.0）
    ★バーの大きさ（＋10／−10）は【設定】（同一ベンチマーク内の比 −18.9 対 ＋62.3 ≒ 0.30 から）。
    ★★02 にこのカードの【金額】の記述は無い。★0円と置いた（→04 §11-3 の②） */
...
/* ── 第4章の決算（02 §1・§7 の M-4）──────────────────────────────────
   st（土台が渡す状態）
     think/obey/width  第3章末のバー      ece  第3章で決まったずれ（pt。★以後変わらない）
     params  第1章のパラメータ数（億）     price 'high'|'std'|'low'（★第3章で決めた分）
     staff   ★通期で雇った人数の累計      prevShare 前年のシェア（%）
     verifier 検証器に回した人数（0〜30）  gold 模範解答を先に入れたか（真偽）
     risk    リスク管理を買った年か（真偽。★毎年の選択）
     capacity その年に買った容量（回）     wThink ★可動域（土台が計算して渡す）
     mul     ★設定9 の倍率（★第3章末の答えの幅で決まる。★この章の考える力の加算に掛かる）
     spent   前年までの累計支出                                                        */
settle(st){
