/* ══════════════════════════════════════════════════════════════════════════
   ch5.data.js ── 第5章「使い方と、誰も分かっていないこと」の数値と式（★設計役の持ち物）
   ──────────────────────────────────────────────────────────────────────────
   ...
   役目   カード定義・値札・バーの加算ポイント・第5章の決算式・出典
   ★置かないもの  解説の本文・画面の文言（ch5.html＝実装役の持ち物）／
                  ★章をまたぐ条件（→04 §11 の一覧。★土台が持つ）
   ★fetch を使わない（→01 §9・§10・04 §0）
   ──────────────────────────────────────────────────────────────────────────
   ★第5章の性質（→02 §4-G-2）
     ・★訓練費0円・運用費で払う章である（第1章＝訓練費で払う章 との対比）
     ・★第4章で RLVR を取ったかどうかで盤面が変わる（★測定された章のまたぎ）
         例を見せる（few-shot） ＝ 効果の符号が ＋10 → −10 に反転する
         道具を呼ぶ             ＝ ★盤面に出さない（押せない）
       → ★この分岐は【章をまたぐ】ので土台が持つ（→04 §11 の①）。data には値だけを置く
     ・★答えの幅の唯一の出口が この章の 温度＋多数決 である（これが無いと飾りバーになる）
     ・★5年目 ＝ 最終年。★歴代表は5年間の累計利益（01 §5）
   ══════════════════════════════════════════════════════════════════════════ */
...
set:{
  ...
  wThink0   :80,       // ★可動域。★凍結値では定数 80（→02 §5-C の A-3''）。★保険＝04 §11-13
...
/* ── カード（第5章。02 §4・§4-G-2）───────────────────────────────────
   ★どれも開発費0円。★払うのは運用費（倍率）である
   think : 考える力への加算ポイント（設定29）  width : 答えの幅への加算（設定32）
   mult  : 原価と設備費に掛かる倍率（設定28）
   turn  : ★第2〜5章はターンで縛らない（02 設定37。→04 §11-3 の①）
   ★★tag と cite は【画面に出る文字列】である（ch5.html が C.src で描画する）。
...
cards:[
 ...
 /* ★道具を呼ぶ ── ★RLVR を取った道では【盤面に出さない】（押せない）
    ★道具の枠組みそのものは確かめた32本の外である（資料も扱っていない）。
    ★語は「このゲームが確かめた論文32本」に統一（→01 §8-1 の②。2026-08-19） */
...
/* ── 第5章の決算（02 §1・§7 の M-4）──────────────────────────────────
   st（土台が渡す状態）
     think/obey/width  第4章末のバー   ece 第3章で決まったずれ（pt）
     params 第1章のパラメータ数（億）   price 'high'|'std'|'low'（第3章で決めた分）
     staff  ★通期で雇った人数の累計    prevShare 前年のシェア（%）
     rlvr   ★第4章で RLVR を取ったか（真偽）★few の符号と道具の可否と倍率に効く
     risk   リスク管理を買った年か      capacity その年に買った容量（回）
     cot/few/tool/temp/maj  この章の選択（真偽）
     wThink ★可動域（土台が計算して渡す）
     mul    ★設定9 の倍率（★第4章末の答えの幅で決まる。★この章の考える力の加算に掛かる）
     spent  前年までの累計支出                                                        */
settle(st){
 const s=this.set;
 ...
 /* ★捌けるのは【その年に買った容量まで】（★正本は 02 §7 の M-4 の1。★買わない年は 0）。
    ★2026-08-20 まで買わない年を【上限なし】として扱っていた（→04 §15）。★値は作っていない */
 ...
 return{
  ...
  /* ★歴代表は5年間の累計利益（01 §5）。★累計は土台が持つ（この章は5年目の1年分だけを返す） */
  final:true
 };
}
};
